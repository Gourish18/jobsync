...
import { Briefcase, Home, Info, LogIn, LogOut, Menu, User, X } from "lucide-react";
...
const Navbar = () => {
  ...
  const { isAuth, user, setIsAuth, setUser,logoutUser,loading } = useAppData();
  const toggleMenu = () => {
    setIsOpen(!isOpen);
  };
  // const isAuth = false;
                ...
                <PopoverTrigger asChild>
                  <button className="flex items-center gap-2 hover:opacity-80 transition-opacity">
                    <Avatar className="h-9 w-9 ring-2 ring-offset-2 ring-offset-background ring-blue-500/20 cursor-pointer hover:ring-blue-500/40 transition-all">
                      <AvatarImage src={user?user.profile_pic as string:""} alt={user?user.name as string:""} />
